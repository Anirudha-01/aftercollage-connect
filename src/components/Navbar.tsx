      ...
      duration: 0.6
        ...
        <div className="container-custom px-4 flex items-center justify-between">
          ...
          <div className="hidden md:block">
            <a href="#cta" className="px-6 py-2.5 gradient-bg text-primary-foreground font-semibold rounded-xl text-sm hover:opacity-90 transition-opacity">
              Get Started
            </a>
          </div>
      ...
      <AnimatePresence>
        ...
        duration: 0.2
            ...
            <div className="flex flex-col gap-4">
              {navLinks.map(link => <a key={link.name} href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-medium hover:text-primary transition-colors">
                  {link.name}
                </a>)}
              <a href="#cta" onClick={() => setIsMobileMenuOpen(false)} className="mt-4 px-6 py-3 gradient-bg text-primary-foreground font-semibold rounded-xl text-center">
                Get Started
              </a>
            </div>
          </motion.div>}
